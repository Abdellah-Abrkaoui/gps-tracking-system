...
const VehicleDashboard = ({ vehicles }) => {
  ...
  const calculateStats = () => {
    ...
    const distance = (speeds.reduce((sum, speed) => sum + speed, 0) / 3600) * (selectedVehicle.speedData.length * 60);
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      <header className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Vehicle History Tracker</h1>
        <p className="text-gray-600">Select a vehicle to view its speed and location history.</p>
      </header>
      ...
      {selectedVehicle ? (
        <div className={`transition-opacity duration-300 ${isLoading ? 'opacity-50' : 'opacity-100'}`}>
              ...
              {stats && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 w-full md:w-auto mt-4 md:mt-0">
                  ...
                  <div className="bg-gray-50 p-3 rounded-md">
                    ...
                    <p className="text-lg font-semibold">{stats.avgSpeed} km/h</p>
                  </div>
                  <div className="bg-gray-50 p-3 rounded-md">
                    <div className="flex items-center mb-1">
                      <Gauge className="h-4 w-4 text-gray-500 mr-1" />
          ...
          {stats && (
            <div className="bg-white p-4 rounded-lg shadow-md mb-6">
              <h2 className="text-lg font-semibold mb-4 text-gray-800">Journey Details</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center">
                  <Calendar className="h-5 w-5 text-blue-800 mr-2" />
                  <div>
                    <p className="text-sm text-gray-500">Start Time</p>
                    <p className="font-medium">{stats.startTime.toLocaleString()}</p>
                  </div>
                </div>
                <div className="flex items-center">
                  <Calendar className="h-5 w-5 text-blue-800 mr-2" />
                  <div>
                    ...
                    <p className="font-medium">{stats.endTime.toLocaleString()}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="bg-blue-50 p-8 rounded-lg text-center">
          <p className="text-lg text-blue-800">Please select a vehicle to view its history.</p>
        </div>
      )}
    </div>
  );
